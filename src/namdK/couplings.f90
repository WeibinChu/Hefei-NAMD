...
!    allocate(olap_sec%DijI(olap_sec%NBANDS, olap_sec%NBANDS, olap_sec%TSTEPS-1))
    ...
    if (lcoup) then
      ...
      if (inp%LCPTXT) then
        ...
        write(*,*) "This version does not support coupling from COUPCAR, please use NATXT_RE and NATXT_IM"
        stop
  ...
  subroutine readNaEigCpx(olap_sec, inp)
    implicit none
    ...
    open(unit=42, file='NATXT_RE', status='unknown', action='read', iostat=ierr)
    if (ierr /= 0) then
      write(*,*) "NATXT_RE does NOT exist!"
      stop
    end if
    
    open(unit=43, file='NATXT_IM', status='unknown', action='read', iostat=ierr)
    if (ierr /= 0) then
      write(*,*) "NATXT_IM does NOT exist!"
      stop
    end if
  ...
  subroutine initspace(olap_sec, inp)
    ...
    type(overlap), intent(inout) :: olap_sec
    type(overlap) :: olap_acbas
    type(namdInfo), intent(in) :: inp
    integer :: i, j, k, N, ierr

    olap_acbas%NBANDS = inp%NACBASIS
